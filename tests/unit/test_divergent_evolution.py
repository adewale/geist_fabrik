"""Unit tests for divergent_evolution geist."""
...
def vault_with_linked_notes(tmp_path):
    """Create a vault with linked notes and session history."""
    ...
    # Create additional notes to meet minimums
    for i in range(30):
...
@pytest.fixture
def vault_no_links(tmp_path):
    """Create a vault without any links."""
    vault_path = tmp_path / "vault"
    vault_path.mkdir()
...
def test_divergent_evolution_suggestion_structure(vault_with_linked_notes):
    ...
    for suggestion in suggestions:
        ...
        # Should reference 2 notes (diverging linked pair)
...
def test_divergent_evolution_insufficient_sessions(vault_insufficient_sessions):
    """Test that divergent_evolution handles insufficient sessions gracefully."""
...
def test_divergent_evolution_no_links(vault_no_links):
    """Test that divergent_evolution handles vault with no links gracefully."""
    vault, session = vault_no_links
...
def test_divergent_evolution_deterministic_with_seed(vault_with_linked_notes):
    ...
    suggestions1 = divergent_evolution.suggest(context1)
    suggestions2 = divergent_evolution.suggest(context2)
