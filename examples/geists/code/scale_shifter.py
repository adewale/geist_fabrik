        ...
        # Highly concrete note - suggest zooming out
            ...
            if abstract_neighbors:
                ...
                text = (
                    ...
                    f"What if you zoomed out? [[{framework.title}]] might provide a broader framework "
                    f"for understanding what makes this case interesting."
                )
        ...
        for concrete in vault.sample(concrete_notes, min(10, len(concrete_notes))):
            if vault.similarity(abstract, concrete) > 0.6:
                if not vault.links_between(abstract, concrete):
                    text = (
                        f"[[{abstract.title}]] (abstract/theoretical) and [[{concrete.title}]] (concrete/specific) "
                        f"are semantically similar but operate at different scales. "
                        f"Could one illuminate the other?"
                    )
