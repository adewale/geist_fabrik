...
theme or area of interest worth naming and organizing.
"""
...
def suggest(vault: "VaultContext") -> list["Suggestion"]:
    ...
    # Sample some notes and find their neighborhoods
    seed_notes = vault.sample(notes, k=5)

    for seed in seed_notes:
        # Get neighbors of this note
        neighbors = vault.neighbors(seed, k=5)

        if len(neighbors) < 3:
            continue

        # Check if these neighbors are also similar to each other
        # (indicating a cluster, not just a hub-and-spoke)
        cluster_notes = [seed] + neighbors[:3]
        ...
        if avg_similarity > 0.6:
            ...
            text = (
                f"What if you recognized an emerging cluster around [[{seed.title}]]? "
                f"These notes are tightly related: [[{formatted_titles}]]. "
                f"Could they be organized under a shared theme?"
            )
