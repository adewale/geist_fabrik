        ...
        for note in vault.sample(notes, min(50, len(notes))):
            ...
            if drift > 0.15:  # Threshold for significant drift
                ...
                if days_since_modified > 30:  # Content hasn't changed recently
                    text = (
                        f"Your understanding of [[{note.title}]] shifted significantly "
                        f"between last session and this one, even though you haven't edited it in {days_since_modified} days. "
                        f"What changed in how you're reading it?"
                    )
                else:
                    text = (
                        f"[[{note.title}]] is being interpreted quite differently in this session—"
                        f"meaning evolving as you edit it?"
                    )
...
def _get_session_history(vault: "VaultContext", sessions_back: int = 5) -> list:
...
def _get_note_embedding_for_session(vault: "VaultContext", note_path: str, session_id: int):
...
def _calculate_drift(emb1, emb2) -> float:
